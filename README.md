...
8 篇内置文章均在正文末尾提供默认折叠的全文中英对照，英文原段与人工校对中文逐段对应。

阅读器另含约 5.8 万条开放许可基础英汉学习词条。点击单词后可听系统 UK / US 发音，并在完成猜测后逐层查看词形、英文释义和中文辅助；基础词义不会冒充当前句义。Settings 可试听并选择当前设备中的英式/美式声音与语速。
...
- Library 可以导入英文纯文本，也可选填与英文空行分段一一对应的整篇中文译文；Dashboard 会按本地日期轮换每日推荐。
- 内置和本地导入文章都支持点词发音与基础词典。词库按小块获取并缓存，查询过的分片可离线复用。
- Dashboard 与 Learning history 的单条理解痕迹可以确认删除；相关猜测和练习会同步清理，学习摘要按剩余证据重算。
